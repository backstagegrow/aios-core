...
async function checkData() {
    const { data: bms, error: err1 } = await supabase.from('business_managers').select('*');
    console.log("BMs in DB:", bms);

    const { data: accounts, error: err2 } = await supabase.from('ad_accounts').select('*');
