...
**Não sabe qual agente usar? → `@aios-master`**

`@aios-master` é o front-door canônico do sistema. Faz triage e roteia para o agente, chief ou squad correto com contexto completo. Use diretamente um agente específico apenas quando já souber exatamente o que precisa.

> Regra completa de roteamento: `.claude/rules/agent-authority.md`

### Ativação de Agentes
Use `@agent-name` ou `/AIOS:agents:agent-name`:

#### Agentes AIOS — Story-Driven Development
...
| Chief | Domínio | Squad Orquestrado |
|-------|---------|-------------------|
| `@copy-chief` | Copywriting & Persuasão | 24 copywriters lendários |
| `@cyber-chief` | Cybersecurity | 6 especialistas de segurança |
| `@data-chief` | Analytics & Métricas | 7 especialistas de dados |
| `@design-chief` | Design & Marca | 9 especialistas de design |
