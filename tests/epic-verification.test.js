...
describe('Epic Verification - Core Logic (Mocked)', () => {
  ...
  describe('Scenario 2: Epic Not Found', () => {
...
describeIfClickUp('Epic Verification - Integration (Live)', () => {
  test('should verify connection to ClickUp API', async () => {
    // This will only run if ClickUp is configured
    // It attempts to verify Epic 1. If Epic 1 doesn't exist or is in an invalid state,
    // verifyEpicExists will throw an error, which is a valid test outcome for connection.
    // A successful connection means the function runs without a connection error.
    try {
      const result = await verifyEpicExists(1);
      expect(result).toBeDefined();
      // Further assertions can be made if Epic 1 is expected to exist and be valid
      // For now, just ensuring the call doesn't fail due to connection issues.
    } catch (error) {
      // If Epic 1 is not found or has an invalid status, that's still a successful API call.
      // We just need to ensure it's not a network/auth error.
      expect(error.message).not.toContain('ClickUp API:'); // Basic check for API errors
      expect(error.message).not.toContain('Network timeout');
      // If it throws for "Epic 1 not found" or "invalid status", that's fine for a connection test.
    }
  });
});
